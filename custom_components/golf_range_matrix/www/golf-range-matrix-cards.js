...
class NovaShotTracerCard extends HTMLElement {
  ...
  render() {
    ...
    // Perspective mowing stripes that fill the fairway triangle (apex 195,24 =
    // far horizon, base = near tee), plus a couple of distance flags, to make
    // the range read like real turf instead of a neon grid.
    const fxL = (y) => 52 + 143 * (242 - y) / 218;
    const fxR = (y) => 338 - 143 * (242 - y) / 218;
    const bandYs = [242, 214, 178, 142, 106, 70, 36, 24];
    let stripes = '';
    ...
    const flag = (y, scale) => {
      const poleH = 13 * scale;
      return `<g class="flag"><line x1="195" y1="${y}" x2="195" y2="${(y - poleH).toFixed(1)}"/><path d="M195 ${(y - poleH).toFixed(1)} l${(8 * scale).toFixed(1)} ${(2.6 * scale).toFixed(1)} l${(-8 * scale).toFixed(1)} ${(2.6 * scale).toFixed(1)} Z"/><ellipse class="green" cx="195" cy="${y}" rx="${(11 * scale).toFixed(1)}" ry="${(3.4 * scale).toFixed(1)}"/></g>`;
    };
    const flags = `${flag(178, 1)}${flag(106, 0.78)}`;
    this.innerHTML = `<ha-card><div class="shell">
      ...
      <div class="stage"><svg viewBox="0 0 390 252" preserveAspectRatio="xMidYMid meet" aria-label="Animated shot tracer driving range grid"><defs><linearGradient id="${tracerId}" x1="0" x2="1" y1="1" y2="0"><stop offset="0" stop-color="#f7ff5c"/><stop offset="0.44" stop-color="#38f8ff"/><stop offset="1" stop-color="#b36bff"/></linearGradient><linearGradient id="sky-${this._renderId}" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#0b1a3a"/><stop offset="1" stop-color="#274a78"/></linearGradient><linearGradient id="rough-${this._renderId}" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#1b3d24"/><stop offset="1" stop-color="#0a1c11"/></linearGradient><radialGradient id="haze-${this._renderId}" cx="0.5" cy="0.16" r="0.6"><stop offset="0" stop-color="rgba(150,210,255,.32)"/><stop offset="1" stop-color="rgba(150,210,255,0)"/></radialGradient><filter id="${glowId}"><feGaussianBlur stdDeviation="4" result="blur"/><feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge></filter></defs>
        <rect class="rough" x="0" y="0" width="390" height="252" fill="url(#rough-${this._renderId})"/><rect class="sky" x="0" y="0" width="390" height="30" fill="url(#sky-${this._renderId})"/><rect class="haze" x="0" y="6" width="390" height="60" fill="url(#haze-${this._renderId})"/><line class="horizon" x1="0" y1="29.5" x2="390" y2="29.5"/>${stripes}<path class="fairwayEdge" d="M52 242 L195 24 L338 242 Z"/>${flags}<path class="target" d="M195 24 V236"/><path class="roughBoundary" d="M104 236 L195 24 M286 236 L195 24"/><path class="grid" d="M22 214 H368 M34 178 H356 M48 142 H342 M66 106 H324 M88 70 H302 M112 36 H278"/>
