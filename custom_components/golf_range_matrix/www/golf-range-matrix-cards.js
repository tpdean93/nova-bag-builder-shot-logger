...
class NovaShotTracerCard extends HTMLElement {
  ...
  set hass(hass) {
    ...
    // While projecting full-screen, keep the aim view stable: a re-render would
    // rebuild the DOM and drop out of fullscreen. We catch up on exit.
    if (this._projecting && document.fullscreenElement) return;
    this.render();
  }
  ...
  // Fairway width in yards (centre-out). ~40 yd is a standard tour fairway; we
  // use it to draw the fairway to scale and to grade each shot as fairway/miss.
  get _fairway() { return Object.assign({ width: 40 }, this._lsGet('nova_fairway', {})); }
  set _fairway(v) { this._lsSet('nova_fairway', v); }
  _fairwayAction(action) {
    const f = this._fairway;
    if (action === 'w+') f.width = this.clamp(f.width + 5, 15, 90);
    else if (action === 'w-') f.width = this.clamp(f.width - 5, 15, 90);
    else if (action === 'reset') f.width = 40;
    this._fairway = f;
    this.render();
  }
  _toggleProjector() {
    ...
    const req = stage.requestFullscreen?.() || Promise.reject();
    Promise.resolve(req).catch(() => { this._projecting = false; });
  }
  _evalWedge(hass) {
    const w = this._wedge;
    if (!w.on) return;
  ...
  render() {
    ...
    // --- Fairway width (centre-out, in yards) drives the trapezoid + grading ---
    const fairway = this._fairway;
    const halfYds = fairway.width / 2;
    const inFairway = carry ? Math.abs(offline) <= halfYds : null;
    const fairwayResult = inFairway === null ? '' : (inFairway ? 'Fairway' : (offline < 0 ? 'Miss Left' : 'Miss Right'));
    const resultRest = [shotName, offlineLabel, fairwayResult].filter(Boolean).join(' | ');
    // --- Driving range geometry: wide perspective trapezoid (not a pyramid) ---
    // 40 yd is our reference width (factor 1.0); scale the edges to the setting.
    const wf = this.clamp(fairway.width / 40, 0.45, 2.0);
    const halfBase = this.clamp(143 * wf, 60, 185);
    const halfTop = this.clamp(39 * wf, 16, 72);
    const TOP_Y = 36, BOT_Y = 242, SPAN = BOT_Y - TOP_Y;
    const BL = 195 - halfBase, BR = 195 + halfBase, TL = 195 - halfTop, TR = 195 + halfTop;
    ...
    const wedgeBar = `<div class="wedgeBar">
      <div class="wstep fwstep"><span class="wlab">Fairway</span><button data-fairway="w-">&minus;</button><b>${fairway.width} yd</b><button data-fairway="w+">+</button></div>
      <button class="wbtn proj" data-proj="1"><ha-icon icon="mdi:projector-screen-outline"></ha-icon><span>Projector</span></button>
    ...
    this.querySelectorAll('[data-fairway]').forEach(btn => btn.addEventListener('click', () => this._fairwayAction(btn.dataset.fairway)));
    this.querySelectorAll('[data-proj]').forEach(btn => btn.addEventListener('click', () => { novaPulse(btn); this._toggleProjector(); }));
    ...
    if (!this._fsBound) {
      ...
      document.addEventListener('fullscreenchange', () => {
        if (!document.fullscreenElement && this._projecting) { this._projecting = false; this.render(); }
      });
    }
  }
}
